...
OFFICIAL_SCORE_COLUMNS = [
    ...
    "3DPrinter AUC (source)", "AirCompressor AUC (source)", "Scanner AUC (source)", "ToyCircuit AUC (source)", "HoveringDrone AUC (source)", "HairDryer AUC (source)", "ToothBrush AUC (source)", "RoboticArm AUC (source)", "BrushlessMotor AUC (source)",
    "3DPrinter AUC (target)", "AirCompressor AUC (target)", "Scanner AUC (target)", "ToyCircuit AUC (target)", "HoveringDrone AUC (target)", "HairDryer AUC (target)", "ToothBrush AUC (target)", "RoboticArm AUC (target)", "BrushlessMotor AUC (target)",
]
...
SCORE_COLUMNS = [
    "official score", "arithmetic mean", "harmonic mean (source)", "harmonic mean (target)",
    "3DPrinter Score (source)", "AirCompressor Score (source)", "Scanner Score (source)", "ToyCircuit Score (source)", "HoveringDrone Score (source)", "HairDryer Score (source)", "ToothBrush Score (source)", "RoboticArm Score (source)", "BrushlessMotor Score (source)",
    "3DPrinter Score (target)", "AirCompressor Score (target)", "Scanner Score (target)", "ToyCircuit Score (target)", "HoveringDrone Score (target)", "HairDryer Score (target)", "ToothBrush Score (target)", "RoboticArm Score (target)", "BrushlessMotor Score (target)",
]
PAPER_OFFICIAL_SCORE_COLUMNS = ["h-mean", "a-mean", "3DPrinter", "AirCompressor", "Scanner", "ToyCircuit", "HoveringDrone", "HairDryer", "ToothBrush", "RoboticArm", "BrushlessMotor"]
    ...
    for machine_idx, target_machine in enumerate(machines):
        ...
        for section_id in section_ids:
            ...
            if not os.path.exists(anomaly_score_path) or \
                ...
                # Load DCASE2024 baseline results
                anomaly_score_path = "{dir}/anomaly_score_DCASE2024T2{machine}_{section}_test_seed{seed}{tag}_Eval.csv".format(
                    dir=target_dir,
                    machine=target_machine,
                    section=section_id,
                    seed=seed,
                    tag=tag)
                decision_result_path = "{dir}/decision_result_DCASE2024T2{machine}_{section}_test_seed{seed}{tag}_Eval.csv".format(
                    dir=target_dir,
                    machine=target_machine,
                    section=section_id,
                    seed=seed,
